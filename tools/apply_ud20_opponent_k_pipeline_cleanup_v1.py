#!/usr/bin/env python3
"""Undefeated 2.0 opponent-K pipeline cleanup.
...
U2-safe scope: opponent-environment base pipeline/card labels only. U2 workload,
K/BF reconciliation, fragility/confidence, BF/IP, probability/distribution,
grading, save/refresh, Savant refresh, Pitching Outs, FS, and Moneyline are untouched.
"""
...
MARKER = "UD20_OPP_K_PIPELINE_CLEANUP_V1_2026_08_28"
...
def _app97_recent_hand_profile(row):
    ...
    with the current lineup once. The legacy recent blend remains audit-only
    and is a fail-safe fallback if the season split is unavailable.
    """
...
def _replace_all_top_level_functions(text: str):
    tree = ast.parse(text)
    nodes = [
    ...
    if not nodes:
        raise RuntimeError(f"No top-level {TARGET_FN} found")
    ...
    for node in sorted(nodes, key=lambda n: n.lineno, reverse=True):
        ...
        if token not in original:
            raise RuntimeError(f"Definition token missing for {TARGET_FN} at line {node.lineno}")
...
def patch_text(text: str):
    if MARKER in text:
        return text, {"already": True, "functions": 0, "cards": 0, "labels": 0}
    out, fn_count = _replace_all_top_level_functions(text)
    card_count = out.count(CARD_OLD)
    label_count = out.count(LINES_OLD)
    if card_count < 1:
        raise RuntimeError("K card opponent-K source block not found")
    if label_count < 1:
        raise RuntimeError("K card opponent-K label block not found")
    out = out.replace(CARD_OLD, CARD_NEW)
    out = out.replace(LINES_OLD, LINES_NEW)
    out = f"# {MARKER}\n" + out
    ast.parse(out)
    return out, {"already": False, "functions": fn_count, "cards": card_count, "labels": label_count}
...
def main():
    ...
    print(f"{MARKER}: {'applied' if updated != original else 'already applied'} {stats}")
